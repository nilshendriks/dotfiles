...
wezterm.on("gui-startup", function(cmd)
    if wezterm.hostname() ~= "mac.home" then
        return
    end
    ...
    local git1 =
        win1:spawn_tab({ cwd = wezterm.home_dir .. "/Sites/storybook-henk" })
    ...
    local git2 =
        win2:spawn_tab({ cwd = wezterm.home_dir .. "/Sites/studio-henk" })
    ...
    leftR:send_text(
        "ssh mini -t 'zsh -i -l -c \"cd /Users/henk3d/HENK-3D/henk-render && clear && exec $SHELL\"'\n"
    )
...
wezterm.on("update-status", function(window, pane)
    ...
    if is_ssh then
        local cmdline =
            table.concat(pane:get_foreground_process_info().argv or {}, " ")
    ...
    window:set_config_overrides(
        is_ssh and { color_scheme = "Fahrenheit" } or {}
    )
end)
