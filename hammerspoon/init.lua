hs.hotkey.bind({ "cmd", "alt", "ctrl" }, "W", function()
    hs.notify
        .new({ title = "Hammerspoon", informativeText = "Hello World" })
        :send()
end)
...
--     hs.alert.show("Active space: " .. index)
...
-- init.lua (snippet)
local spacemgr = require("spacemanager")

-- create a menu item to show current space (optional)
local spaceMenu = hs.menubar.new()
if spaceMenu then
    spaceMenu:setTitle("Space ?")
end

-- start the manager
spacemgr.start(spaceMenu)

-- optionally keep a hotkey to re-evaluate (works if you change machineLayouts on the fly)
-- hs.hotkey.bind({ "cmd", "alt", "ctrl" }, "R", function()
--     hs.reload()
-- end)
